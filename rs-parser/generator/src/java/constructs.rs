use crate::java::constants::{Type, ArrayType};
use std::cmp;
...
pub enum Attribute {
  ...
  /// ```
  Code {max_stack: u16, operations: Vec<Operation>, locals: u16}
}
...
pub fn count_local_vars(
  signature: Option<&Signature>,
  operations: &Vec<Operation>) -> u16 {
  let arg_count = signature.map(|s| s.parameter_types.len() as u16).unwrap_or(0);
  let op_count = operations.iter()
    .map(|op| match op {
...
pub fn merge_codes(
  opt_signature: Option<&Signature>,
  mut codes: Vec<Attribute>) -> Attribute {
  let initial_locals: u16 = opt_signature.map(|s| s.parameter_types.len() as u16).unwrap_or(0);
  codes.drain(0..)
    .fold(
      Attribute::Code {max_stack: 0, operations: vec![], locals: initial_locals},
      |mut r, mut e| {
      {
        let Attribute::Code {max_stack: max, operations: ref mut ops, locals} = e;
        let &mut Attribute::Code {max_stack: ref mut result_max, operations: ref mut result_ops, locals: ref mut result_locals } = &mut r;
        *result_max += max;
...
mod tests {
  ...
  fn test_merge_codes_with_signature() {
    ...
    let Attribute::Code {locals: locals1, max_stack: _, operations: _} = merge_codes(
      Some(&Signature {
        return_type: Type::Void,
        parameter_types: vec![
          Type::Integer,
    ...
    let Attribute::Code {locals: locals2, max_stack: _, operations: _}  = merge_codes(
      Some(&Signature {
        return_type: Type::Void,
        parameter_types: vec![
          Type::Integer,
